...
// Itens AAMA que são INVERTIDOS (escore = 4 - resposta_bruta)
const ITENS_AAMA_INVERTIDOS = new Set([1, 5, 8, 10, 14, 16, 27, 29, 42]);
...
function calcularEscores() {
  ...
  for (const item of ITENS_ETDAH) {
    ...
    if (isNaN(num) || num < 0 || num > 4) {
      alert(`Item ${item.n}: valor inválido "${val}". Use 0 a 4.`);
      return null;
    }
  ...
  for (const item of ITENS_ETDAH) {
    ...
    // Inversão AAMA
    const ajustado = ITENS_AAMA_INVERTIDOS.has(item.n) ? (4 - raw) : raw;
...
function renderRelatorio(d) {
  ...
  <div style="background:#eff6ff;border-radius:10px;padding:14px 16px;border:1px solid #bfdbfe;margin-bottom:24px;">
    <p style="font-size:11.5px;color:#1e40af;line-height:1.7;margin:0;">
      <strong>Nota técnica:</strong> O ETDAH-AD é um instrumento de autoavaliação de sintomas de TDAH para adultos (12–87 anos).
